...
class DBCheckpoint:
    ...
    def save(self, state: DBState) -> None:
        if self.mode == 'sqlite':
    ...
    def load_latest(self, thread_id: str) -> DBState:
        if self.mode == 'sqlite':
            assert self.connection is not None
...
def main() -> None:
    ...
    db_path = Path(__file__).with_name(f'runtime_checkpoints{suffix}')
    ...
    try:
        ...
        print('Latest:', store.load_latest('order-9'))
    finally:
        store.close()
