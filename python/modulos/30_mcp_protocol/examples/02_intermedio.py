...
class RichMCPServer:
    ...
    def handle_resources_read(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "contents": [{"uri": document.uri, "mimeType": "text/plain", "text": document.text}]
        }
    ...
    def handle_prompts_list(self, _: dict[str, Any]) -> dict[str, Any]:
        return {
            "prompts": [
                {"name": name, "description": template} for name, template in self.prompts.items()
            ]
        }
...
def run_simulation() -> str:
                ...
                json.dumps(
                    {
                        ...
                        "params": {
                            "name": "compare_topics",
                            "arguments": {"left": "stdio", "right": "HTTP+SSE"},
                        },
                    }
                ),
            ]
    ...
    for line in incoming:
        ...
        outgoing.write(
            json.dumps({"request": message, "response": response}, ensure_ascii=False) + "\n"
        )
