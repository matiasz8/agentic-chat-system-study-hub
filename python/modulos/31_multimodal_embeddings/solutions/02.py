...
class VectorStore:
    ...
    def _build_embedder(self, query: str) -> tuple[list[str], dict[str, float]]:
        ...
        total = len(corpus)
        idf = {
            token: math.log((1 + total) / (1 + sum(1 for text in corpus if token in set(tokenize(text))))) + 1
...
def main() -> None:
    ...
    store.add_document('doc-c', 'Una consulta textual puede recuperar imágenes si el espacio es compartido.')
    store.delete('doc-b')
